...
import {
  Compass, Search, Filter, BookOpen, Layers, CheckCircle2,
  Clock, Eye, Download, Bookmark, ChevronRight, Grid, List,
  Play, FileText, BarChart3, CheckSquare, Sparkles, X, Plus
} from 'lucide-react';
...
interface LearningPageProps {
  ...
  /** Supervisor view: the catalogue is something to inspect, not to enrol in. */
  reviewMode?: boolean;
}
...
export function LearningPage({
  ...
  const levels: { id: EducationLevel; label: string; count: number }[] = [
    ...
    { id: 'SD', label: 'Sekolah Dasar (SD)', count: materials.filter(m => m.level === 'SD').length },
    { id: 'SMP', label: 'SMP / Sederajat', count: materials.filter(m => m.level === 'SMP').length },
    { id: 'SMA', label: 'SMA / SMK / Sederajat', count: materials.filter(m => m.level === 'SMA').length },
  ];

  // Format Types
  const formats: { id: MaterialType; label: string; icon: any }[] = [
    { id: 'all', label: 'Semua Format', icon: Layers },
    { id: 'video', label: 'Video Pembelajaran', icon: Play },
    { id: 'modul', label: 'Modul PDF & Dokumen', icon: FileText },
    { id: 'infografis', label: 'Infografis & Visual', icon: BarChart3 },
    { id: 'kuis', label: 'Kuis & Evaluasi', icon: CheckSquare },
  ];
  ...
  return (
    <div className="space-y-6 pb-12">
      {/* 1. HERO HEADER LEARNING LIBRARY */}
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-1.5 max-w-2xl">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-50 border border-blue-100 text-blue-800 text-xs font-semibold">
              ...
              <span>Pusat Kurikulum & Modul Nasional</span>
            </div>
            <h1 className="font-headline text-2xl sm:text-3xl font-bold text-slate-900 tracking-tight">
              {reviewMode ? 'Katalog Materi' : 'Learning Library'}
            </h1>
            <p className="text-xs sm:text-sm text-slate-600 leading-relaxed">
              {reviewMode
                ? 'Daftar materi keselamatan berlalu lintas, kesadaran hukum, dan pencegahan kriminalitas yang dipakai instruktur di lapangan. Buka satu materi untuk meninjau silabus dan soal evaluasinya.'
                : 'Jelajahi materi keselamatan berlalu lintas, kesadaran hukum, dan pencegahan kriminalitas usia dini hingga remaja. Pilih kursus untuk melihat silabus lengkap.'}
            </p>
          </div>
      ...
      <div className="space-y-4">
        ...
        <div className="bg-white rounded-xl border border-slate-200/80 p-3 sm:px-4 flex flex-wrap items-center justify-between gap-3 shadow-2xs">
          ...
          <div className="flex items-center gap-2 ml-auto">
            ...
            {(selectedType !== 'all' || sortBy !== 'latest' || selectedLevel !== 'ALL' || searchQuery) && (
              <button
                onClick={() => {
                  onSelectType('all');
                  onSelectSort('latest');
      ...
      {filteredList.length === 0 && (
        <div className="bg-white rounded-2xl border border-slate-200 p-12 text-center max-w-lg mx-auto my-8 shadow-xs">
          ...
          <p className="text-xs text-slate-500 mb-6">
            Tidak ada silabus atau modul keselamatan yang sesuai dengan kriteria filter Anda.
          </p>
          <button
            onClick={() => {
              onSelectLevel('ALL');
            ...
            >
              ...
              <div className="h-44 relative bg-slate-100 overflow-hidden">
                ...
                <button
                  ...
                  className={`absolute top-3 right-3 p-1.5 rounded-full backdrop-blur-sm transition-colors cursor-pointer ${
                    mat.bookmarked
              ...
              <div className="p-4 flex flex-col flex-1">
                ...
                <div className="mt-auto pt-3 border-t border-slate-100 flex items-center justify-between text-xs">
                  ...
                  <span className="font-semibold text-blue-700 group-hover:translate-x-0.5 transition-transform flex items-center gap-0.5">
                    Silabus <ChevronRight className="w-3.5 h-3.5" />
                  </span>
                </div>
              </div>
            </div>
          ...
          {filteredList.map((mat) => (
            ...
            >
              ...
              <div className="flex items-center gap-2 shrink-0 self-end sm:self-center" onClick={e => e.stopPropagation()}>
                ...
                >
                  <span>{reviewMode ? 'Tinjau Materi' : 'Lihat Detail Kursus'}</span>
